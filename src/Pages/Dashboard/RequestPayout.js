...
// Submit to formspree
    fetch(process.env.REACT_APP_formSpree, {
        ...
        body: JSON.stringify({...userInfo, Subject: "AfroLoom Partner Payout Request"}),
  });      reset();
      toastRef.current.show({ severity: 'success', summary: 'Request sent successfully!', detail: 'Payout is being processed, you will hear from us shortly'});
    } catch (error) {
      toastRef.current.show({ severity: 'error', summary: 'Error sending request', detail: error.message });
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="container">
        ...
        <div className="container mb-5 mt-5 d-flex justify-content-center rounded">
          <form className="col-12 col-sm-6" onSubmit={handleSubmit(onSubmit)}>
            ...
            />
            {errors.mobileMoneyNumber && <p className="text-danger">{errors.mobileMoneyNumber.message}</p>}
            ...
            />
            {errors.accountName && <p className="text-danger">{errors.accountName.message}</p>}
            ...
            <div className="form-group mb-3">
              ...
              />
              {errors.serviceProvider && <p className="text-danger">{errors.serviceProvider.message}</p>}
            </div>
