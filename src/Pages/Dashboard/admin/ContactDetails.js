...
const ContactDetails = () => {
  ...
  useEffect(() => {
    const fetchContacts = async () => {
      try {
        ...
        setContacts(response.docs.map(doc => ({ ...doc.data(), id: doc.id })));
      } catch (error) {
  ...
  const addItem = (field) => {
    setSelectedContact({ ...selectedContact, [field]: [...selectedContact[field], ''] });
  };
  ...
  const updateItem = (e, field, index) => {
    const updatedField = selectedContact[field].map((item, i) => (i === index ? e.target.value : item));
    setSelectedContact({ ...selectedContact, [field]: updatedField });
  };
  ...
  const saveContact = async () => {
    setIsLoading(true);
    try {
      await AllServices.updatecontactDetail(selectedContact.id, selectedContact);
      ...
      toast.current.show({ severity: 'error', summary: 'Error', detail: 'Failed to update contact', life: 3000 });
    }
    setIsLoading(false);
  };
  ...
  const renderEditDialog = () => {
    ...
    return (
      <Dialog className='w-75' header="Edit Contact Details" visible={!!selectedContact}  modal onHide={() => setSelectedContact(null)}>
        {['call', 'email', 'location'].map(field => (
          <div key={field} className="p-field">
            ...
            <button onClick={() => addItem(field)}   className="btn btn-primary text-white w-100 mb-4 mt-2">
        ...
        <div className="p-dialog-footer">
          <button onClick={saveContact} type="submit" className="btn btn-warning text-white w-100 mt-4 shadow-sm position-relative">
  ...
  return (
    <div className="p-m-3">
      ...
      <DataTable value={contacts} paginator rows={10} rowsPerPageOptions={[5, 10, 25]} className="p-datatable-gridlines">
        <Column field="call" header="Call" body={renderArrayField('call')} />
        <Column field="email" header="Email" body={renderArrayField('email')} />
        <Column field="location" header="Location" body={renderArrayField('location')} />
        <Column
          header="Actions"
